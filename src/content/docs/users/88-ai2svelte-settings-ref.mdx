...
Whether to prioritize parent width, height or cover. Setting to cover will make the graphics respect it's aspect ratio and fill the entire parent element, potentially cropping parts of the graphic if the aspect ratios do not match. Setting to height will make the graphic adapt to the height of its parent element instead of the width.

Options: `width`, `height`, `cover`

:::caution
If set to height or cover, parent element must have a defined height and width. Might lead to unexpected results if the parent element has no defined height.
:::
...
:::caution
inline_svg: true does not carry blend modes. Use only for simple graphics.
:::
