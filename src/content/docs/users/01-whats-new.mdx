...
- `:png`/`:png24` layers are now exported with whitespace trimmed. This significantly reduces the size of the exported images.
- fixes a bug that caused issues with exporting `:svg` layers

### v1.0.7

- resolves MRAP issues that popup while exporting SVG with newer versions of Illustrator
