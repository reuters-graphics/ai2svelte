...
:::note
It is advised to get the best result for blending modes off `:png`/`:png24` layers, apply blending mode directly to the parent layer since `:png`/`:png24` tagged layers are rasterised together with the child elements.
:::
...
:::note
Stacking multiple blend modes between parent and child in `:svg` tagged layers are inconsistent in how Adobe Illustrator and Web implements it.

For safe result, let the parent layer be blended as NORMAL and apply blending modes to child elements.
:::
