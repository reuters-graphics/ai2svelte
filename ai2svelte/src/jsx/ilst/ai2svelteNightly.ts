...
export function main() {
  ...
  function render(settings, customBlocks) {
    ...
    forEachUsableArtboard(function (activeArtboard, abIndex) {
      ...
      var textFrames, textData, imageData, blockData;
  ...
  function exportSymbolAsHtml(item, geometries, abBox, opts) {
    ...
    var properties = item.name
      ...
      : "";
    var geom, x, y;
    var innerBlock = opts.innerBlock + "\r" || "";
    // CSS for blocks to cover full parent size
    var innerBlockClass = "";

    if (opts.innerBlock) {
      innerBlockClass = " g-block-layer";
    }
  ...
  function exportSymbols(lyr, ab, masks, opts) {
    ...
    function forPageItem(item) {
      ...
      if (opts.isCustomBlock) {
        ...
        forEach(lyr.textFrames, function (x) {
          if (x.name.split(":")[0] == "props") {
            if(x.name.split(":")[1] != undefined) {
              if(docSettings.customIncludes) {
                docSettings.customIncludes.push(x.name.split(":")[1]);
              } else {
                docSettings.customIncludes = [];
                docSettings.customIncludes.push(x.name.split(":")[1]);
              }
            }
            
            innerBlock += x.contents;
          }
        });

        // replaces smart codes to minimal straight quotes
        innerBlock = innerBlock.toString().replace(/[\u201C\u201D]/g, '"');
        innerBlock = innerBlock.toString().replace(/[\u2018\u2019]/g, '"');
  ...
  function convertArtItems(activeArtboard, textFrames, masks, settings) {
    ...
    forEachImageLayer("png", function (lyr) {
      ...
      layerHtml.push({
        z: lyrZ,
        html: exportImage(name, "png", activeArtboard, null, null, opts) + "\r"
      });
      hiddenLayers.push(lyr); // need to unhide this layer later, after base image is captured
    });

    forEach(findTaggedLayers("block").reverse(), function (lyr) {
      var lyrZ = lyr.zOrderPosition;
      docSettings.hasCustomBlocks = true;
    ...
    html =
      captureArtboardImage(imgName, activeArtboard, masks, settings) + accumulatedHTML.join('');
  ...
  function generatePageCss(containerId, settings) {
    ...
    if (docSettings.hasCustomBlocks) {
      css += customBlockCss();
    }
  ...
  function getResizerScript(containerId, settings) {
    ...
    if(docSettings.customIncludes) {
      svelteJS += "import { " + docSettings.customIncludes.join(",") + " } from '@reuters-graphics/graphics-components';\r";
    }
