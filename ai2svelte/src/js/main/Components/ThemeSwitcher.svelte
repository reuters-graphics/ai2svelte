<script lang="ts">
  ...
  // Sync the data-theme attribute to the root element whenever theme changes.
  // This drives all theme-aware CSS variables defined in global stylesheets.
  $effect(() => {
    if (theme && document) {
...
<!--
  ...
  - Keyboard activation: Space / Enter toggles the pressed state
  The child snippet lets us apply the tooltip Svelte action on the button directly,
  since actions can only be used on native HTML elements, not component wrappers.
-->
<Toggle.Root
  pressed={theme === "dark"}
  onPressedChange={(pressed) => (theme = pressed ? "dark" : "light")}
>
  {#snippet child({ props })}
    <button
      {...props}
